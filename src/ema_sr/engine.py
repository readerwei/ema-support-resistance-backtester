...
def scan_ema_periods(
    ...
    for period in periods:
        summary = analyze_ema_interactions(bars, period, atr_period, atr_multiple).summary
        rows.append({
            "ema_period": period,
...
def _best_bounce_pct(bars: pd.DataFrame, ema_periods: Iterable[int], atr_period: int, atr_multiple: float) -> float:
    scores = [analyze_ema_interactions(bars, p, atr_period, atr_multiple).summary["combined_bounce_pct"] for p in ema_periods]
...
def monte_carlo_p_value(
    ...
    for _ in range(simulations):
        ...
        if _best_bounce_pct(synthetic, ema_periods, atr_period, atr_multiple) >= actual_bounce_pct:
            at_least_as_high += 1
